...
# Test 6: Error in pipeline doesn't lose other queries
with_mariadb(sub {
    my @results;
    $m->q("select 'ok1'", sub {
        my ($rows, $err) = @_;
    ...
    $m->q("select 'ok2'", sub {
        my ($rows, $err) = @_;
        push @results, $err ? "err:$err" : "ok:$rows->[0][0]";
        # After error, pipeline connection is invalidated,
        # so we check what we got
        is($results[0], 'ok:ok1', 'pipeline: first query ok before error');
